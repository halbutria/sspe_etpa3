...
namespace SispeServicios.Api.Parametrico.Aplicacion.CrudInactivacionCuenta
{
    public class Edicion
    {
        ...
        public class Manejador : IRequestHandler<Ejecuta, ParametricoDTO>
        {
            ...
            {
                var info = await _contexto.InactivacionCuenta.FindAsync(Int32.Parse(request.Id));

                if (info is not null)
                {
                    _mapper.Map(request, info, typeof(Ejecuta), typeof(Modelo.InactivacionCuenta));

                    _contexto.InactivacionCuenta.Update(info);
