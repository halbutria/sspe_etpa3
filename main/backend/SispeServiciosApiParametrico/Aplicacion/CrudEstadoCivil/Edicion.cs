...
namespace SispeServicios.Api.Parametrico.Aplicacion.CrudEstadoCivil
{
    public class Edicion
    {
        ...
        public class Manejador : IRequestHandler<Ejecuta, ParametricoDTO>
        {
            ...
            {
                var info = await _contexto.estadoCivil.FindAsync(Int32.Parse(request.Id));

                if (info is not null)
                {
                    _mapper.Map(request, info, typeof(Ejecuta), typeof(Modelo.EstadoCivil));

                    _contexto.estadoCivil.Update(info);
