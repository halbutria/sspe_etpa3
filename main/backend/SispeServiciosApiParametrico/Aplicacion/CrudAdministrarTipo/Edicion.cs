...
namespace SispeServicios.Api.Parametrico.Aplicacion.CrudAdministrarTipo
{
    public class Edicion
    {
        ...
        public class Manejador : IRequestHandler<Ejecuta, ParametricoDTO>
        {
            ...
            {
                var info = await _contexto.AdministrarTipo.FindAsync(Int32.Parse(request.Id));

                if (info is not null)
                {
                    _mapper.Map(request, info, typeof(Ejecuta), typeof(Modelo.AdministrarTipo));

                    _contexto.AdministrarTipo.Update(info);
