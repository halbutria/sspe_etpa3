...
namespace SispeServicios.Api.Parametrico.Aplicacion.CrudCriterioVideo
{
    public class Edicion
    {
        ...
        public class Manejador : IRequestHandler<Ejecuta, ParametricoDTO>
        {
            ...
            {
                var info = await _contexto.CriterioVideo.FindAsync(Int32.Parse(request.Id));

                if (info is not null)
                {
                    _mapper.Map(request, info, typeof(Ejecuta), typeof(Modelo.CriterioVideo));

                    _contexto.CriterioVideo.Update(info);
