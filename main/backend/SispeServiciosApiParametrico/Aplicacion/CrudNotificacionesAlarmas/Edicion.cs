...
namespace SispeServicios.Api.Parametrico.Aplicacion.CrudNotificacionesAlarmas
{
    public class Edicion
    {
        ...
        public class Manejador : IRequestHandler<Ejecuta, ParametricoDTO>
        {
            ...
            {
                var info = await _contexto.NotificacionesAlarmas.FindAsync(Int32.Parse(request.Id));

                if (info is not null)
                {
                    _mapper.Map(request, info, typeof(Ejecuta), typeof(Modelo.NotificacionesAlarmas));

                    _contexto.NotificacionesAlarmas.Update(info);
