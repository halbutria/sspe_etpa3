...
namespace SispeServicios.Api.Parametrico.Aplicacion.CrudPaisInternacional
{
    public class Edicion
    {
        ...
        public class Manejador : IRequestHandler<Ejecuta, ParametricoDTO>
        {
            ...
            {
                var info = await _contexto.PaisInternacional.FindAsync(Int32.Parse(request.Id));

                if (info is not null)
                {
                    _mapper.Map(request, info, typeof(Ejecuta), typeof(Modelo.Pais));

                    _contexto.PaisInternacional.Update(info);
