...
namespace SispeServicios.Api.Parametrico.Aplicacion.CrudCampoSalidaPlanMejoramiento
{
    public class Edicion
    {
        ...
        public class Manejador : IRequestHandler<Ejecuta, ParametricoDTO>
        {
            ...
            {
                var info = await _contexto.CampoSalidaPlanMejoramiento.FindAsync(Int32.Parse(request.Id));

                if (info is not null)
                {
                    _mapper.Map(request, info, typeof(Ejecuta), typeof(Modelo.CampoSalidaPlanMejoramiento));

                    _contexto.CampoSalidaPlanMejoramiento.Update(info);
