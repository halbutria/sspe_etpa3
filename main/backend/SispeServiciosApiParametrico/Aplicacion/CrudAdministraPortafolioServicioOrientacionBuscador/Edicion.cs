    ...
    public class Edicion
    {
        ...
        public class Manejador : IRequestHandler<Ejecuta, ParametricoDTO>
        {
            ...
            public async Task<ParametricoDTO> Handle(Ejecuta request, CancellationToken cancellationToken)
            {
                ...
                if (info is not null)
                {
                    _mapper.Map(request, info, typeof(Ejecuta), typeof(Modelo.AdministraPortafolioServicioOrientacionBuscador));

                    _contexto.AdministraPortafolioServicioOrientacionBuscador.Update(info);
